...
async def _send_when_ready(
    ...
    *,
    ...
    try:
            ...
            while True:
                if login.done():
                    ...
                    raise AssertionError(
                        "OAuth login completed before its callback listener started"
                    )
                try:
        ...
        cancellation_requested = login.cancel()
        try:
            await login
        except asyncio.CancelledError:
            if not cancellation_requested:
                raise
        raise AssertionError("OAuth callback listener did not start") from None
